...
export default class mainKeywords extends Component {
  ...
  constructor(props) {
    super(props);
    let placeholder = [{translations:[{title:'loading'}]}];
  ...
  render() {
    ...
    return (
      <main>
        ...
        {this.keywords.map((keyword,i)=>(<Link to={"/keywords/"+keyword.id_keyword} key={"keyword"+keyword.id_keyword} id={keyword.id_keyword}>{keyword.translations.map(a => a.title).join(" / ")}</Link>))}
        </main>
    );
  }
}
